...
def main(i):
    ...
    parser.add_argument("--output_dir", default='./CM-BERT_output_{}'.format(i), type=str,
    ...
    nb_tr_steps = 0
    tr_loss = 0
    train_audio,valid_audio,test_audio= pickle.load(open('./data/audio/MOSI_cmu_audio_CLS.pickle','rb'))
    max_acc = 0
    min_loss = 100
    if args.do_train:
    ...
    if args.do_test:
        ...
        model = BertForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir, num_labels = num_labels)
        model.load_state_dict(torch.load('./CM-BERT_output_{}/pytorch_model.bin'.format(i)))
        model.to(device)
        model.eval()
...
if __name__ == "__main__":
    ...
    acc7_list = []
    for i in range(1):
        os.system('mkdir ./CM-BERT_output_{}'.format(i))
        results = main(i)
